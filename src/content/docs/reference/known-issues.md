...
|---|---|---|---|
| Extension images built empty from a full sysroot | A 4 KB BSP image; on the device, the merge fails and no extension service starts | Clean the extension, rebuild, run `scripts/check-avocado-build.sh` before flashing. [Details](../../build/stale-state/#up-to-date-checks-trust-inputs-not-outputs) | Open: [#283](https://github.com/avocado-linux/avocado-cli/issues/283) — [later comment](https://github.com/avocado-linux/avocado-cli/issues/283#issuecomment-5734534010) narrows it to an inline `packages:` map in the project's own extension; a fetched/manifest-only BSP extension was retested and found to populate correctly. [#280](https://github.com/avocado-linux/avocado-cli/issues/280) is related but the reporter's [own follow-up](https://github.com/avocado-linux/avocado-cli/issues/280#issuecomment-5723499727) says it's no longer a correctness bug now that #272 is merged — a performance idea, not a defect. |
...
| `kernel.cmdline`/`cmdline_extra` and the project's `initramfs:` are accepted but never reach the Jetson boot image, on deploy or provision | `/proc/cmdline` is empty of anything you set; kernel-level hardening options can't be applied | None known; a `KernelCommandLine` UEFI variable seeded via `BOOTCONTROL_OVERLAYS` is an untested possibility. [Details](../../hardware/jetson-boot-image/) | Not filed, but related: [meta-avocado#391](https://github.com/avocado-linux/meta-avocado/pull/391) (open) starts repacking the Orin boot.img cmdline with two `avocado.root_partuuid` values for rootfs discovery — not `cmdline_extra` or a general mechanism. [meta-avocado#398](https://github.com/avocado-linux/meta-avocado/issues/398) is hardware debugging from that same PR. |
